    ...
    <section className="py-32">
      <div className="container mx-auto px-4">
        <Tabs defaultValue={defaultTab} className="p-0">
          <TabsList className="bg-background flex h-auto w-full flex-col gap-2 p-0 md:flex-row justify-center">
            {features.map((tab) => {
              return (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className={`hover:border-muted data-[state=active]:bg-muted group flex w-full flex-col items-start justify-between gap-2 whitespace-normal rounded-md border p-4 text-left shadow-none transition-opacity duration-200 hover:opacity-80 data-[state=active]:shadow-none h-[140px] overflow-hidden ${
                    tab.isDefault ? "" : ""
                  }`}
                >
                  ...
                  <p className="text-muted-foreground font-normal transition-opacity duration-200 md:block text-sm leading-tight">
                    {tab.description}
                  </p>
                </TabsTrigger>
              );
          ...
          {features.map((tab) => (
            ...
            >
              <img
                src={tab.image}
                alt={tab.heading}
                className="aspect-video w-full max-w-4xl rounded-md object-cover transition-opacity duration-300"
              />
            </TabsContent>
          ))}
        </Tabs>
